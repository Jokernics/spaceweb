...
export default function CustomSelect({ options, onChange }) {
  ...
  const handleSelect = (option) => {
    // if (option.label === currentValue.label) return
  ...
  return (
    <div className={styles.container}>
      <div onClick={() => setIsOpen(prev => !prev)} className={styles.header}>
        <p className={styles.text}>{currentValue.label}</p>
        <ArrowIcon className={`${styles.arrowIcon} ${openClass}`} />
      </div>
      <div className={`${styles.listContainer} ${hidenClass}`}>
        {options.map((option, i) =>
          <p key={option + i} onClick={() => handleSelect(option)} className={`${styles.text} ${styles.option} ${option.label === currentValue.label ? styles.selected : ''}`}>{option.label}</p>
        )}
      </div>
